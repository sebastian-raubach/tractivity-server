package uk.co.raubach.tractivity.server.database;
...
public class Database
{
	...
	public static void init(String databaseServer, String databaseName, String databasePort, String username, String password, boolean initAndUpdate)
	{
		...
		if (initAndUpdate)
		{
			...
			if (!databaseExists)
			{
				...
				try
				{
					URL url = Database.class.getClassLoader().getResource("uk/co/raubach/tractivity/server/database/init/db_setup.sql");
			...
			try
			{
				...
				Flyway flyway = Flyway.configure()
									  ...
									  .locations("classpath:uk/co/raubach/tractivity/server/database/migration")
			...
			try
			{
				URL url = Database.class.getClassLoader().getResource("uk/co/raubach/tractivity/server/database/init/views_procedures.sql");
	...
	{
		Database.init("localhost", "tractivity", null, "root", null, true);
...
//	}
	...
	public static DSLContext getContext(Connection connection)
	{
		Settings settings = new Settings()
			.withRenderMapping(new RenderMapping()
				.withSchemata(
					new MappedSchema().withInput(TractivityDb.TRACTIVITY_DB.getQualifiedName().first())
									  .withOutput(databaseName)));
